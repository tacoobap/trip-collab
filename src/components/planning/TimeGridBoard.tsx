...
export function TimeGridBoard({
  ...
  /**
   * Phones show one day per screen, changed from the pills: 6.25rem of the
   ...
   * so a day lands flush against it; `handleWrapScroll` reads that same offset
   * back off column 0. Days are changed from the pills alone: the wrapper is
   * `touch-pan-y` below `sm`, so a touch can only ever scroll the hours and the
   * diagonal you make holding a phone one-handed can't page the board. Snapping
   * is how `scrollToDay` settles.
   */
  const canvasStyle = {
    height: canvasH,
    backgroundImage:
